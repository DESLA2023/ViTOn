...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir', type=bool, default="./sample_dataset/model/model_0.jpg", help='Define model(.jpg) file dir # default=\"./sample_dataset/model/model_0.jpg\"')
    parser.add_argument('--cloth_dir', type=bool, default="./sample_dataset/clothes/cloth_0.jpg", help='Define cloth(.jpg) data dir # default=\"./sample_dataset/clothes/cloth_0.jpg\"')
    parser.add_argument('--output_dir', type=bool, default="./result_demo", help='Define output(.jpg) path dir # default=\"./result_demo/\"')
    
    opt = parser.parse_args()
    
    origin_dir = opt.model_dir
    cloth_dir = opt.cloth_dir
    output_dir = opt.output_dir
    
    # Read input(model_origin) image
    img=cv2.imread(origin_dir)
    ori_img=cv2.resize(img,(768,1024))
    cv2.imwrite("{}/origin.jpg".format(output_dir),ori_img)

    # Resize input image
    img=cv2.imread('{}/origin.jpg'.format(output_dir))
    img=cv2.resize(img,(384,512))
    cv2.imwrite('{}/resized_img.jpg'.format(output_dir),img)

    # Get mask of cloth
    print("Get mask of cloth\n")
    terminnal_command = "python get_cloth_mask.py --cloth_dir {} --output_dir {}".format(cloth_dir, output_dir) 
    os.system(terminnal_command)

    # Get openpose coordinate using posenet
    input_path = output_dir + "/origin.jpg"
    print("Get openpose coordinate using posenet\n")
    terminnal_command = "python posenet.py --input_path {} --output_dir {}".format(input_path, output_dir) 
    os.system(terminnal_command)
    ...
    ##############################################
    terminnal_command ="python exp/inference/inference.py --loadmodel ./inference.pth --img_path ../{}/resized_img.jpg --output_path ../{} --output_name /resized_segmentation_img".format(output_dir, output_dir)
    os.system(terminnal_command)
    os.chdir("../")

    # Remove background image using semantic segmentation mask
    mask_img=cv2.imread('{}/resized_segmentation_img.png'.format(output_dir) ,cv2.IMREAD_GRAYSCALE)
    ...
    cv2.imwrite("{}/seg_img.png".format(output_dir),img_seg)
    img=cv2.resize(img_seg,(768,1024))
    # cv2.imwrite('./HR-VITON-main/test/test/image/00001_00.jpg',img)
    cv2.imwrite('{}/00001_00.jpg'.format(output_dir),img)

    # Generate grayscale semantic segmentation image
    terminnal_command ="python get_seg_grayscale.py --output_dir {}".format(output_dir)
    os.system(terminnal_command)
    ...
    {}origin.jpg --output output.pkl -v".format(output_dir)
    os.system(terminnal_command)
